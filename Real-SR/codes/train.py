...
wrtr = SummaryWriter("summary/hc18")
wrva = SummaryWriter("summary/hc18")
        ...
        for _, train_data in enumerate(train_loader):
                ...
                for k, v in logs.items():
                    ...
                    if opt['use_tb_logger'] and 'debug' not in opt['name']:
                        if rank <= 0:
                            ...
                            wrtr.add_scalar(k, v, current_step)
                if rank <= 0:
                    logger.info(message)
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0 and val_loader is not None:
                ...
                if opt['use_tb_logger'] and 'debug' not in opt['name']:
                    ...
                    wrva.add_scalar('psnr', avg_psnr, current_step)
                    wrva.add_scalar('val_pix_err_f', val_pix_err_f, current_step)
                    wrva.add_scalar('val_pix_err_nf', val_pix_err_nf, current_step)
                    wrva.add_scalar('val_mean_color_err', val_mean_color_err, current_step)
                ...
                for k, v in logs.items():
                    ...
                    if opt['use_tb_logger'] and 'debug' not in opt['name']:
                        if rank <= 0:
                            #tb_logger.add_scalar(k, v, current_step)
                            wrva.add_scalar('v_'+k, v, current_step)
                    if rank <= 0:
